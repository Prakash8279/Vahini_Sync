/* ===== Driver Notification & Rerouting View ===== */
import '../styles/driver.css';
import { DRIVER_ALERTS } from '../simulation/data.js';
...
const FLOW_STEPS = [
    {
        title: 'Detection',
        desc: 'Vahini-Sync detects ambulance on emergency route. Nearby drivers within 800m radius are identified via cell-tower mapping.',
        detail: ['Range: 800m', 'Method: Cell-tower + GPS', 'Latency: <200ms'],
        active: false,
    },
    {
        title: 'Initial Alert',
        desc: 'Navigation apps push an overlay alert: "🚑 Ambulance approaching. Keep left." No app installation needed — works via Google/Apple Maps integration.',
        detail: ['Channel: Push overlay', 'Format: Visual + Audio', 'Dismissible: Yes'],
        active: true,
    },
    {
        title: 'Lane Guidance',
        desc: 'Directional arrows guide drivers to clear the ambulance lane. Lane assignment is dynamic based on traffic density.',
        detail: ['Direction: Left/Right', 'Adaptive: Yes', 'Coverage: 500m corridor'],
        active: false,
    },
    {
        title: 'Alternate Route',
        desc: 'Drivers on blocked routes get a suggested alternate route with estimated time difference. Soft redirect — not forced.',
        detail: ['Type: Suggestion', 'Time delta: +2-4 min', 'Compliance: ~72%'],
        active: false,
    },
    {
        title: 'All Clear',
        desc: 'After ambulance passes, drivers receive "✅ Resume normal driving" notification. Signal timing reverts to normal cycle.',
        detail: ['Auto-clear: 15s', 'Signal reset: Gradual', 'Traffic recovery: ~90s'],
        active: false,
    },
];
...
const view = {
        ...
        <div class="driver-flow">
          ...
          <div id="flow-steps">
            ${FLOW_STEPS.map((step, i) => `
              <div class="flow-step ${step.active ? 'active' : ''}" id="flow-step-${i}">
                <div class="flow-step-number">${i + 1}</div>
                <div class="flow-step-content">
                  <h3>${step.title}</h3>
                  <p>${step.desc}</p>
                  <div class="flow-detail">
                    ${step.detail.map(d => `<span>${d}</span>`).join('')}
                  </div>
                  ${i === 2 ? `
    ...
    `;
        ...
        // Play sequence
        document.getElementById('driver-play-btn')?.addEventListener('click', () => {
            this.playSequence();
        });
    },
    ...
    updatePhase() {
        ...
        FLOW_STEPS.forEach((_, i) => {
            ...
            if (el) el.classList.toggle('active', i === alertPhase);
        });
      ...
      `;
            ...
            if (dot) dot.style.background = 'var(--signal-red)';
            if (text) { text.style.color = 'var(--signal-red)'; text.textContent = '8s'; }
      ...
      `;
            ...
            if (text) { text.style.color = 'var(--signal-green)'; text.textContent = '32s'; }
        }
    },
