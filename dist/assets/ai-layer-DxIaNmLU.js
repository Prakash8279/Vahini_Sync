let e=null;const c=[{type:"input",msg:"GPS update: AMB-DL-4721 at 28.5588°N, 77.2430°E | speed: 42km/h"},{type:"input",msg:"Traffic sensor J2: density 92 veh/100m | queue: 14 vehicles"},{type:"input",msg:"Historical model: 6PM peak, Ring Road congestion P=0.95"},{type:"process",msg:"RL Agent: evaluate action space for J3 → extend_green(25s)"},{type:"process",msg:"Graph predictor: flow propagation → J4 impact: +16s side delay"},{type:"output",msg:"Signal command: J3 → GREEN_MAIN [duration: 60s] [priority: EMERGENCY]"},{type:"output",msg:"Driver alert: push to 23 devices in 500m radius [type: YIELD_LEFT]"},{type:"safety",msg:"Safety check: J2 side-road red duration 68s < MAX(120s) ✓ PASS"},{type:"process",msg:"Route optimizer: confidence 94% for Ring Road corridor"},{type:"input",msg:"V2X beacon: cross-traffic count at J4 = 8 vehicles waiting"},{type:"safety",msg:"Override check: no manual override active → AI control retained"},{type:"output",msg:"ETA update: destination in 7m 45s (improved from 14m 30s)"},{type:"process",msg:"Congestion prediction: J5 clear in 12s, pre-empt green wave"},{type:"input",msg:"Network health: latency 12ms, packet loss 0.01%, uplink stable"},{type:"safety",msg:"Auto-cancel check: ambulance moving > 5km/h ✓ emergency valid"}],o={render(a){a.innerHTML=`
    ...
    `,this.startFeed()},startFeed(){const a=document.getElementById("feed-body");if(!a)return;let d=0;const i=()=>{const s=c[d%c.length],t=new Date().toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:!1}),l=document.createElement("div");for(l.className="feed-line",l.innerHTML=`
        <span class="feed-time">${t}</span>
        <span class="feed-type ${s.type}">[${s.type.toUpperCase()}]</span>
        <span class="feed-msg">${s.msg}</span>
      `,a.prepend(l);a.children.length>50;)a.removeChild(a.lastChild);d++};for(let s=0;s<8;s++)i();e=setInterval(i,2e3)},destroy(){e&&clearInterval(e),e=null}};export{o as default};
